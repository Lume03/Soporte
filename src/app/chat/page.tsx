...
// --- CAMBIO 1: Importar useSession ---
import { useSession } from "next-auth/react"; 
import { useState, useEffect } from 'react';
import type { Message } from '@/lib/types';
import { ChatArea } from '@/components/chat/chat-area';
import { ChatInput } from '@/components/chat/chat-input';
import { Bot, Send, Ticket as TicketIcon, User, Building2, UserCog, Briefcase, CalendarDays, CheckCircle, ClipboardList, FileText, CircleDot, Sparkles } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { submitMessage as handleMessageSubmit } from '@/lib/actions';

// --- (El resto de tus componentes internos como GeneratedTicketCard y HomePage permanecen IGUAL) ---
...
        <div className="flex-1 flex items-center justify-center p-6 min-h-[calc(100vh-88px)]">
            <div className="w-full max-w-4xl bg-white rounded-2xl p-12 text-center mx-auto shadow-xl">
                ...
                <h1 className="text-3xl mb-6 leading-relaxed">
                    <span className="bg-gradient-to-r from-[#3498DB] via-[#2980B9] to-[#1ABC9C] bg-clip-text text-transparent font-bold">¡Hola! Soy el Asistente Virtual de Analytics.</span>
                </h1>
                ...
                <form onSubmit={handleSubmit} className="relative max-w-2xl mx-auto">
                    ...
                    <input type="text" placeholder="Escribe tu pregunta..." value={inputValue} onChange={(e) => setInputValue(e.target.value)} disabled={isLoading}
                    ...
                    <Button type="submit" disabled={isLoading || !inputValue.trim()} size="icon"
                        ...
                        {isLoading ? (<div className="h-6 w-6 animate-spin rounded-full border-2 border-white border-t-transparent" />) : (<Send className="h-6 w-6" />)}
                    </Button>
                </form>
            </div>
        </div>
...
// --- Componente Principal de la Página de Chat (AQUÍ ESTÁN LOS CAMBIOS) ---
export default function ChatPage() {
    // --- CAMBIO 2: Obtener la sesión (que incluye nuestro token de backend) ---
...
    }, []);
    ...
    // Esta es la función principal que se actualiza
...
        setIsLoading(true);
        ...
        // --- CAMBIO 3: Obtener y validar el token de FastAPI desde la sesión de NextAuth ---
        // @ts-ignore
        ...
        if (!token) {
            ...
            setIsLoading(false);
            setIsChatLocked(true); // Bloqueamos el chat si falla la autenticación
            return; // Detenemos la ejecución
        }
        // --- FIN DEL CAMBIO 3 ---

        try {
            // --- CAMBIO 4: Pasar el 'token' como TERCER argumento a la acción ---
...
                // Tu lógica de generación de ticket (esto parece ser mockeado, lo cual está bien por ahora)
                setTimeout(() => generateTicket(messageContent), 1000);
            }
        } catch (error) {
            console.error("Error devuelto por handleMessageSubmit:", error);
            const errorMessage: Message = { id: crypto.randomUUID(), role: 'assistant', content: "Lo siento, ha ocurrido un error. Voy a generar un ticket para que nuestro equipo te ayude." };
            setMessages(prev => [...prev, errorMessage]);
...
    };
    ...
    // Tu lógica de ticket (parece mockeada, así que no se toca)
    const generateTicket = (userQuery: string) => {
        const ticket = {
            id: `TCK-2025-${String(Math.floor(Math.random() * 90000) + 10000).padStart(5, 'о')}`,
            type: 'Incidencia', user: 'Ana González', company: 'Scotiabank', analyst: 'Equipo de Data Science',
            subject: userQuery, service: 'Data Science', status: 'Abierto',
...
    }
    ...
    // El resto de tu JSX que renderiza ChatArea y ChatInput (no necesita cambios)
    return (
...
// --- TIPO PARA LOS DETALLES DEL TICKET ---
type TicketDetailItem = {
    icon: React.ElementType;
    label: string;
    value: string;
...
// --- Componente para la Tarjeta de Ticket (SIN CAMBIOS) ---
function GeneratedTicketCard({ ticket }: { ticket: any }) {
    const details: TicketDetailItem[] = [
        { icon: ClipboardList, label: 'Tipo', value: ticket.type },
        { icon: User, label: 'Usuario', value: ticket.user },
...
// --- Componente Principal de la Página de Chat (CON LOS CAMBIOS APLICADOS) ---
export default function ChatPage() {
    const [messages, setMessages] = useState<Message[]>([]);
    const [isLoading, setIsLoading] = useState(false);
    const [showHomePage, setShowHomePage] = useState(true);
    ...
    const submitMessage = async (messageContent: string) => {
        ...
        try {
            const response = await handleMessageSubmit(messageContent, threadId);
            setThreadId(response.thread_id);
